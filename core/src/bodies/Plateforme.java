...
public class Plateforme {
	...
	body = world.createBody(bodyDef);
        ...
        direction = new Vector2(ligne[etape].x - body.getPosition().x, ligne[etape].y - body.getPosition().y);
	...
	public void deplacement(){
		if(!boucle){
			if(!retour){
				if(!new Vector2(ligne[etape].x - body.getPosition().x, ligne[etape].y - body.getPosition().y).hasSameDirection(direction)){
					etape++;

			        if(etape == ligne.length){
				retour = true;
				etape = ligne.length - 2;
			        }

					direction.set(ligne[etape].x - body.getPosition().x, ligne[etape].y - body.getPosition().y);
				}
			}
			else{
				if(!new Vector2(ligne[etape].x - body.getPosition().x, ligne[etape].y - body.getPosition().y).hasSameDirection(direction)){
	...
	public void draw(SpriteBatch batch, TextureAtlas textureAtlas, Couleurs couleurs){
		batch.setColor(couleurs.getCouleurLeger());
