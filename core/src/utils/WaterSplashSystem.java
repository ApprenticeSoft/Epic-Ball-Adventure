...
public class WaterSplashSystem {
	...
	private static final String BUBBLE_RING_REGION = "BubbleRing";
	...
	private static final float SURFACE_SMOOTHING = 0.14f;
	private static final float SURFACE_MAX_ADJACENT_DELTA = 0.34f;
	private static final float SURFACE_BUSY_ENERGY = 1.25f;
	private static final float SURFACE_BUSY_MIN_IMPACT_AMPLITUDE = 0.28f;
	private static final int MAX_BUSY_TRAVELING_WAVES = 22;
	...
	private static final float MAX_BUBBLE_TRAIL_WINDOW = 1.65f;
	private static final float BUBBLE_MIN_RISE_ACCEL = 0.65f;
	...
	private final Vector2 bubbleFixtureLocalPoint = new Vector2();
	...
	public void enterWater(Eau water, Fixture impactFixture, Contact contact){
		...
		registerSubmergedFixture(water, impactFixture, impact.downwardSpeed, impact.size);
	...
	public void drawBubbles(SpriteBatch batch, TextureAtlas textureAtlas, Color waterColor){
		drawBubblesBehindWater(batch, textureAtlas, waterColor);
	}

	public void drawBubblesBehindWater(SpriteBatch batch, TextureAtlas textureAtlas, Color waterColor){
		if(bubbles.size == 0)
			return;
		TextureRegion ringRegion = findBubbleRingRegion(textureAtlas);
		TextureRegion highlightRegion = findParticleCircleRegion(textureAtlas);
		if(ringRegion == null)
			return;
		...
		for(AirBubble bubble : bubbles){
			...
			setBubbleRingColor(alpha, bubbleColor);
			batch.setColor(bubbleColor);
			batch.draw(ringRegion,
			...
			if(highlightRegion == null)
				continue;
			float highlightRadius = radius * 0.15f;
			setBubbleHighlightColor(capToWaterAlpha(alpha * 0.38f, waterAlpha), bubbleHighlightColor);
			batch.setColor(bubbleHighlightColor);
			batch.draw(highlightRegion,
	...
	public void spawnDebugBubbleProbe(){
		if(waters == null || waters.size == 0)
			return;
		Eau water = waters.first();
		float waterAlpha = waterAlpha(water);
		int count = 14;
		for(int i = 0; i < count; i++){
			float ratio = count <= 1 ? 0.5f : i / (float)(count - 1);
			float localX = MathUtils.lerp(-water.getSurfaceHalfWidth() * 0.38f,
					water.getSurfaceHalfWidth() * 0.38f, ratio);
			float surfaceY = surfaceLocalY(water, localX);
			float radius = 0.42f + (i % 4) * 0.075f;
			float depth = minimumBubbleSpawnDepth(radius) + 0.16f + (i % 3) * 0.16f;
			float localY = MathUtils.clamp(surfaceY - depth, -water.height + BUBBLE_SURFACE_MARGIN,
					surfaceY - BUBBLE_SURFACE_MARGIN);
			bubbleWorldPoint.set(water.body.getWorldPoint(bubbleWorldPoint.set(localX, localY)));
			Vector2 velocity = new Vector2(MathUtils.random(-0.08f, 0.08f), MathUtils.random(0.22f, 0.48f));
			bubbles.add(AirBubble.create(water, bubbleWorldPoint, velocity, radius,
					capToWaterAlpha(waterAlpha * 0.96f, waterAlpha), 7f));
		}
		trimBubbles();
		DebugConfig.log("water bubble probe count=" + count + " total=" + bubbles.size);
	}
	...
	private void registerSubmergedFixture(Eau water, Fixture fixture, float downwardSpeed, float size){
		...
		for(SubmergedFixture submerged : submergedFixtures){
			if(submerged.fixture == fixture){
				...
				submerged.startBubbleTrail(downwardSpeed, size);
				return;
			}
		}
		SubmergedFixture submerged = new SubmergedFixture(water, fixture);
		submerged.startBubbleTrail(downwardSpeed, size);
		submergedFixtures.add(submerged);
	}
	...
	private void updateSubmergedBubbleTrails(float delta){
		...
		for(int i = submergedFixtures.size - 1; i >= 0; i--){
			...
			if(!submerged.isBubbleTrailActive())
				continue;
			submerged.trailWindow = Math.max(0f, submerged.trailWindow - delta);
			float descentSpeed = Math.max(0f, -body.getLinearVelocity().dot(submerged.water.getSurfaceNormal()));
			if(submerged.trailWindow <= 0f || descentSpeed < MIN_BUBBLE_TRAIL_DESCENT_SPEED){
			...
			for(int bubbleIndex = 0; bubbleIndex < bubbleCount; bubbleIndex++)
				spawnTrailBubble(submerged.water, fixture, descentSpeed, mass, size);
	...
	private void spawnTrailBubble(Eau water, Fixture fixture, float speed, float mass, float size){
		...
		if(!sampleSubmergedBubblePoint(water, fixture, radius, bubbleWorldPoint))
			return;
		float waterAlpha = waterAlpha(water);
		float alpha = capToWaterAlpha(waterAlpha * MathUtils.random(0.2f, 0.6f), waterAlpha);
	...
	private boolean sampleSubmergedBubblePoint(Eau water, Fixture fixture, float radius, Vector2 out){
		if(water == null || fixture == null || fixture.getBody() == null || out == null)
			return false;
		Shape shape = fixture.getShape();
		if(shape == null)
			return false;
		for(int attempt = 0; attempt < MAX_BUBBLE_SPAWN_SAMPLE_ATTEMPTS; attempt++){
			if(!sampleFixtureLocalPoint(shape, bubbleFixtureLocalPoint))
				return false;
			bubbleCandidateWorldPoint.set(fixture.getBody().getWorldPoint(bubbleFixtureLocalPoint));
			if(!fixture.testPoint(bubbleCandidateWorldPoint))
				continue;
			if(!isBubbleSpawnWorldPointSubmerged(water, bubbleCandidateWorldPoint, radius))
				continue;
			out.set(bubbleCandidateWorldPoint);
			return true;
		}
		return false;
	}

	private boolean sampleFixtureLocalPoint(Shape shape, Vector2 out){
		if(shape.getType() == Shape.Type.Circle)
			return sampleCircleLocalPoint((CircleShape)shape, out);
		if(shape.getType() == Shape.Type.Polygon)
			return samplePolygonLocalPoint((PolygonShape)shape, out);
		return false;
	}

	private boolean sampleCircleLocalPoint(CircleShape circle, Vector2 out){
		float angle = MathUtils.random(0f, MathUtils.PI2);
		float distance = circle.getRadius() * (float)Math.sqrt(MathUtils.random());
		Vector2 center = circle.getPosition();
		out.set(center.x + MathUtils.cos(angle) * distance, center.y + MathUtils.sin(angle) * distance);
		return true;
	}

	private boolean samplePolygonLocalPoint(PolygonShape polygon, Vector2 out){
		int vertexCount = polygon.getVertexCount();
		if(vertexCount <= 0)
			return false;
		polygon.getVertex(0, bubblePolygonVertex);
		float minX = bubblePolygonVertex.x;
		float maxX = bubblePolygonVertex.x;
		float minY = bubblePolygonVertex.y;
		float maxY = bubblePolygonVertex.y;
		for(int i = 1; i < vertexCount; i++){
			polygon.getVertex(i, bubblePolygonVertex);
			minX = Math.min(minX, bubblePolygonVertex.x);
			maxX = Math.max(maxX, bubblePolygonVertex.x);
			minY = Math.min(minY, bubblePolygonVertex.y);
			maxY = Math.max(maxY, bubblePolygonVertex.y);
		}
		out.set(MathUtils.random(minX, maxX), MathUtils.random(minY, maxY));
		return true;
	}

	private boolean isBubbleSpawnWorldPointSubmerged(Eau water, Vector2 worldPoint, float radius){
		bubbleCandidateWaterPoint.set(water.body.getLocalPoint(worldPoint));
		return isBubbleSpawnLocalPointSubmerged(bubbleCandidateWaterPoint.x, bubbleCandidateWaterPoint.y,
				surfaceLocalY(water, bubbleCandidateWaterPoint.x), water.getSurfaceHalfWidth(), water.height, radius);
	}
	...
	static float calculateBubbleTrailRadiusBase(float descentSpeed, float mass, float size){
		float safeMass = Math.max(0.02f, mass);
		float safeSize = Math.max(0.1f, size);
	...
	static float calculateBubbleTrailRadiusMin(float descentSpeed, float mass, float size){
		float speedScale = calculateBubbleTrailSpeedScale(descentSpeed);
		return calculateBubbleTrailRadiusBase(descentSpeed, mass, size) * MathUtils.lerp(0.48f, 0.62f, speedScale);
	}
	...
	static float minimumBubbleSpawnDepth(float radiusBase){
		return Math.max(0.62f, Math.max(0f, radiusBase) * 1.6f + BUBBLE_SURFACE_MARGIN);
	}

	static float bubbleSpawnSurfaceClearance(float radius){
		return BUBBLE_SURFACE_MARGIN + bubbleRemovalClearance(radius) + 0.01f;
	}

	static boolean isBubbleSpawnLocalPointSubmerged(float localX, float localY, float surfaceY, float halfWidth,
			float height, float radius){
		return localX >= -Math.max(0f, halfWidth)
				&& localX <= Math.max(0f, halfWidth)
				&& localY <= surfaceY - bubbleSpawnSurfaceClearance(radius)
				&& localY >= -Math.max(0f, height) + BUBBLE_SURFACE_MARGIN;
	}

	static float calculateBubbleTrailRate(float totalSpeed, float mass, float size){
		float speed = Math.max(0f, totalSpeed);
		if(speed < MIN_BUBBLE_TRAIL_DESCENT_SPEED)
			return 0f;
		float safeMass = Math.max(0.02f, mass);
		float safeSize = Math.max(0.1f, size);
	...
	static float calculateBubbleTrailWindow(float downwardSpeed, float size){
		return MathUtils.clamp(0.42f + Math.max(0f, downwardSpeed) * 0.055f + Math.max(0.1f, size) * 0.1f,
				0.42f, MAX_BUBBLE_TRAIL_WINDOW);
	}

	static float calculateBubbleRiseAcceleration(float radius){
		return MathUtils.clamp(0.5f + Math.max(0f, radius) * 4.4f,
				BUBBLE_MIN_RISE_ACCEL, BUBBLE_MAX_RISE_ACCEL);
	}

	static float calculateBubbleMaxRiseSpeed(float radius){
		return MathUtils.clamp(0.72f + Math.max(0f, radius) * 6.4f,
				BUBBLE_MIN_RISE_SPEED, BUBBLE_MAX_RISE_SPEED);
	}

	static boolean bubbleReachedSurface(float localY, float surfaceY, float radius){
		return localY + bubbleRemovalClearance(radius) >= surfaceY - BUBBLE_SURFACE_MARGIN;
	}

	static float calculateBubbleSurfaceFade(float depth, float radius){
		float fadeDepth = Math.max(0.1f, Math.max(0.01f, radius) * 1.25f);
		float surfaceClearance = Math.max(0f, depth - BUBBLE_SURFACE_MARGIN);
		return smoothStep(MathUtils.clamp(surfaceClearance / fadeDepth, 0f, 1f));
	}

	static float bubbleRemovalClearance(float radius){
		return Math.max(0f, radius) * 0.35f;
	}
	...
	static String bubbleRingRegionName(){
		return BUBBLE_RING_REGION;
	}
	...
	private static TextureRegion findBubbleRingRegion(TextureAtlas textureAtlas){
		if(textureAtlas == null)
			return null;
		TextureRegion region = textureAtlas.findRegion(BUBBLE_RING_REGION);
		return region == null ? textureAtlas.findRegion(PARTICLE_CIRCLE_REGION) : region;
	}

	static Color setBubbleRingColor(float alpha, Color out){
		Color target = out == null ? new Color() : out;
		target.set(0.96f, 1f, 1f, MathUtils.clamp(alpha, 0f, 1f));
		return target;
	}

	static Color setBubbleShadowColor(Color waterColor, float alpha, Color out){
		Color target = out == null ? new Color() : out;
		float waterRed = waterColor == null ? 0f : waterColor.r;
		float waterGreen = waterColor == null ? 0.45f : waterColor.g;
		float waterBlue = waterColor == null ? 0.65f : waterColor.b;
		target.set(MathUtils.clamp(waterRed * 0.12f, 0f, 1f),
				MathUtils.clamp(waterGreen * 0.32f, 0f, 1f),
				MathUtils.clamp(waterBlue * 0.42f + 0.05f, 0f, 1f),
				MathUtils.clamp(alpha, 0f, 1f));
		return target;
	}
	...
	static final class SubmergedFixture {
		...
		float bubbleDebt;
		float trailWindow;
		private boolean bubbleTrailActive;
		private boolean bubbleTrailConsumed;
		...
		void startBubbleTrail(float downwardSpeed, float size){
			if(bubbleTrailConsumed)
				return;
			bubbleTrailConsumed = true;
			if(downwardSpeed < MIN_BUBBLE_TRAIL_DESCENT_SPEED)
				return;
			bubbleTrailActive = true;
			trailWindow = calculateBubbleTrailWindow(downwardSpeed, size);
			bubbleDebt = 0f;
		}

		void finishBubbleTrail(){
			bubbleTrailActive = false;
			trailWindow = 0f;
			bubbleDebt = 0f;
		}

		boolean isBubbleTrailActive(){
			return bubbleTrailActive;
		}
	}
	...
	static final class AirBubble {
		...
		static AirBubble create(Eau water, Vector2 position, Vector2 velocity, float radius, float alpha,
				float lifetime){
			return create(water, position, velocity, radius, alpha, lifetime,
					calculateBubbleRiseAcceleration(radius), calculateBubbleMaxRiseSpeed(radius));
		}

		static AirBubble create(Eau water, Vector2 position, Vector2 velocity, float radius, float alpha,
				float lifetime, float riseAcceleration, float maxRiseSpeed){
			...
			bubble.riseAcceleration = MathUtils.clamp(riseAcceleration, BUBBLE_MIN_RISE_ACCEL,
					BUBBLE_MAX_RISE_ACCEL);
			bubble.maxRiseSpeed = MathUtils.clamp(maxRiseSpeed, BUBBLE_MIN_RISE_SPEED, BUBBLE_MAX_RISE_SPEED);
		...
		void update(float delta){
			age += delta;
			velocity.y = Math.min(maxRiseSpeed, velocity.y + riseAcceleration * delta);
	...
	static final class WaterSurfaceSimulation {
		...
		void applyImpact(float localX, float amplitude, float size, float alpha, int visibleWaveCount){
			float admissionScale = impactAdmissionScale(amplitude);
			if(admissionScale <= 0f)
				return;
			amplitude *= admissionScale;
			visibleWaveCount = MathUtils.round(visibleWaveCount * (0.45f + admissionScale * 0.55f));
		...
		private void spawnTravelingWaves(float localX, float amplitude, float size, int visibleWaveCount){
			if(visibleWaveCount <= 0 || amplitude <= 0.01f)
				return;
		...
		private float impactAdmissionScale(float amplitude){
			float waveBusy = MathUtils.clamp((travelingWaves.size - MAX_BUSY_TRAVELING_WAVES)
					/ (float)MAX_BUSY_TRAVELING_WAVES, 0f, 1f);
			float energyBusy = MathUtils.clamp((energy - SURFACE_BUSY_ENERGY) / SURFACE_BUSY_ENERGY, 0f, 1f);
			float busy = Math.max(waveBusy, energyBusy);
			if(busy > 0.15f && amplitude < SURFACE_BUSY_MIN_IMPACT_AMPLITUDE)
				return 0f;
			float impactStrength = MathUtils.clamp(amplitude / 0.8f, 0f, 1f);
			return MathUtils.clamp(1f - busy * (1f - impactStrength) * 0.88f, 0.2f, 1f);
		}

		private void smoothSpringSurface(){
			if(sampleCount < 3)
				return;
			leftDeltas[0] = displacements[0];
			leftDeltas[sampleCount - 1] = displacements[sampleCount - 1];
			for(int i = 1; i < sampleCount - 1; i++){
				float smoothed = (displacements[i - 1] + displacements[i] * 2f + displacements[i + 1]) * 0.25f;
				leftDeltas[i] = MathUtils.lerp(displacements[i], smoothed, SURFACE_SMOOTHING);
			}
			for(int i = 0; i < sampleCount; i++)
				displacements[i] = leftDeltas[i];
		}

		private void limitAdjacentSpringDelta(){
			for(int i = 1; i < sampleCount; i++)
				displacements[i] = limitNeighborDelta(displacements[i - 1], displacements[i]);
			for(int i = sampleCount - 2; i >= 0; i--)
				displacements[i] = limitNeighborDelta(displacements[i + 1], displacements[i]);
		}

		private float limitNeighborDelta(float reference, float value){
			return MathUtils.clamp(value, reference - SURFACE_MAX_ADJACENT_DELTA,
					reference + SURFACE_MAX_ADJACENT_DELTA);
		}

		private void removeWeakestTravelingWave(){
			if(travelingWaves.size == 0)
				return;
			int weakestIndex = 0;
			float weakestEnergy = travelingWaves.first().energy();
			for(int i = 1; i < travelingWaves.size; i++){
				float waveEnergy = travelingWaves.get(i).energy();
				if(waveEnergy < weakestEnergy){
					weakestEnergy = waveEnergy;
					weakestIndex = i;
				}
			}
			travelingWaves.removeIndex(weakestIndex);
		}
